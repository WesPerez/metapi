...
function createSessionAccount(id: number, name = `session-${id}`) {
  return {
    id,
    siteId: 10,
    username: name,
    accessToken: `session-token-${id}`,
    apiToken: `sk-session-${id}`,
    status: 'active',
...
describe('Accounts segmented connections view', () => {
  ...
  it('paginates session accounts with the same controls as apikey connections', async () => {
    apiMock.getAccounts.mockResolvedValue(
      Array.from({ length: 16 }, (_, index) => (
        createSessionAccount(index + 1, `session-${String(index + 1).padStart(2, '0')}`)
      )),
    );
    apiMock.getSites.mockResolvedValue([
      { id: 10, name: 'Session Site', platform: 'new-api', status: 'active' },
    ...
    try {
      ...
      const pageSizeSelect = root.root.find(
        (node) => node.type === 'select' && node.props['aria-label'] === '每页数量',
      );
      await act(async () => {
        pageSizeSelect.props.onChange({ target: { value: '30' } });
      });
      await flushMicrotasks();
      ...
      expect(collectText(root.root)).toContain('显示第 1 - 16 条，共 16 条');
    } finally {
      root?.unmount();
    }
  });
