...
The build sets `VITE_CHECKIN_MODE=true`, so the UI exposes only account management and the password, schedule, and system-proxy settings. The runtime sets `CHECKIN_APP_MODE=true`, which keeps the check-in scheduler active while disabling balance refresh, daily summary, log cleanup, polling, OAuth callback, backup, and proxy-retention background jobs.
