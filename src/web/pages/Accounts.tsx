...
export default function Accounts() {
  ...
  const effectiveConnectionPage = Math.min(
    Math.max(connectionPage, 1),
    connectionTotalPages,
  );
  const displayedAccounts = useMemo(() => {
    const start = (effectiveConnectionPage - 1) * connectionPageSize;
    return visibleAccounts.slice(start, start + connectionPageSize);
  ...
  useEffect(() => {
    ...
    if (targetIndex >= 0) {
      const targetPage = Math.floor(targetIndex / connectionPageSize) + 1;
      if (targetPage !== effectiveConnectionPage) {
    ...
    visibleAccounts.length > 0 ? (
      ...
      >
        <div style={{ fontSize: 13, color: "var(--color-text-muted)" }}>
          显示第 {(effectiveConnectionPage - 1) * connectionPageSize + 1} -{" "}
          {Math.min(
        ...
        >
          ...
          <button
            ...
            onClick={() =>
              setConnectionPage((page) =>
                Math.min(connectionTotalPages, page + 1),
              )
            }
            disabled={effectiveConnectionPage === connectionTotalPages}
            className="btn btn-ghost"
            style={{ padding: "6px 12px", fontSize: 13 }}
          >
            下一页
          ...
          <button
            type="button"
            onClick={() => setConnectionPage(connectionTotalPages)}
            disabled={effectiveConnectionPage === connectionTotalPages}
            className="btn btn-ghost"
            style={{ padding: "6px 12px", fontSize: 13 }}
          >
          ...
          <select
            ...
            onChange={(e) => {
              setConnectionPageSize(Number.parseInt(e.target.value, 10) || 15);
              setConnectionPage(1);
            }}
            style={{
              padding: "6px 10px",
            ...
            aria-label="每页数量"
          >
            {CONNECTION_PAGE_SIZE_OPTIONS.map((size) => (
