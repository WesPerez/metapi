...
describe('Login surface', () => {
  ...
  it('renders a poster-style hero with a floating admin login panel', () => {
    ...
    try {
      ...
      expect(pageText).toContain('账号刷新 · 模型获取 · 定时签到');
      expect(pageText).toContain('账号签到管理');
      expect(pageText).toContain('定时任务');
      expect(pageText).toContain('系统代理');
